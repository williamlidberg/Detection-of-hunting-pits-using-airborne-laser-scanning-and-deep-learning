# This code file is using code from the following source: https://github.com/WongKinYiu/yolov7, which is licensed under the GNU General Public License v3.0 (GPL-3.0).
...
from tifffile import imread #use instead of cv2
        ...
        if self.video_flag[self.count]:
           ...
           # pdb.set_trace()
            ...
            if path.endswith('.tiff') or path.endswith('.tif'):
                img0 = imread(path)  #changed from: img0 = cv2.imread(path)  # BGR
            else:
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    ...
    def __getitem__(self, index):
        ...
        if len(img.shape) < 3: # for 1 channel tiff image
            img = np.expand_dims(img, 0)

        # Makes a copy of the numpy array to avoid negative strides
        img = np.copy(img)
    ...
    if img is None:  # not cached
        ...
        if path.endswith('.tiff') or path.endswith('.tif'): #using tifffile instead of cv2 for tiffs
            ...
            if (img.shape[2] >= 3):
                ...
                if img.shape[2] > 3:
                    t1 = cv2.split(img_rgb)
                    t2 = cv2.split(img)[3:]
                    img = cv2.merge(t1 + t2)
                else:
                    img = img_rgb

            #img = img[..., ::-1] # Swap channels in last dimensions
...
def letterbox(img, new_shape=(640, 640), color=(114, 114, 114), auto=True, scaleFill=False, scaleup=True, stride=32):
    ...
    if shape[::-1] != new_unpad:  
        img_resized = np.zeros((new_unpad[1], new_unpad[0], img.shape[2]), dtype= np.uint8)
    ...
    # Padding
    img_padded = np.zeros((new_shape[0], new_shape[1], img.shape[2]), dtype=np.uint8)
    for i in range(img.shape[2]):
        img_padded[...,i] = cv2.copyMakeBorder(img[...,i], top, bottom, left, right, cv2.BORDER_CONSTANT, value=color[i%3])  # add border
    img = img_padded

    # returns the padded/resized image, a ratio of the scaling factor that was used, the amount of padding applied to each side of the img
...
def extract_boxes(path='../coco/'):  # from utils.datasets import *; extract_boxes('../coco128')
    ...
    for im_file in tqdm(files, total=n):
        if im_file.suffix[1:] in img_formats:
            ...
            #im = im[..., ::-1] # swap channels in last dimension
