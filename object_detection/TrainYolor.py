# This code file is using code from the following source:https://github.com/WongKinYiu/yolor, which is licensed under the GNU General Public License v3.0 (GPL-3.0).
# The code is being used in accordance with the terms of the GPL-3.0 license. 
#
# The full text of the GPL-3.0 license can be found at: https://www.gnu.org/licenses/gpl-3.0.en.html
#

# Modified section for our own implementation
import sys
...
import TestYolor as test  # import test.py to get mAP after each epoch
#from models.yolo import Model
#from models.models import *
from YolorModels import *
from utils.autoanchor import check_anchors
# Modified section for our own implementation
#from utils.datasets import create_dataloader
from DatasetMultiChannelYolor import create_dataloader
...
from lossYolor import compute_loss
from PlotsYolor import plot_images, plot_labels, plot_results, plot_evolution
...
def train(hyp, opt, device, tb_writer=None, wandb=None):
    ...
    if pretrained:
        ...
        # Modified section for our own implementation
        model = Darknet(opt.cfg,img_size=(opt.img_size,opt.img_size)).to(device)  # create
        state_dict = {k: v for k, v in ckpt['model'].items() if model.state_dict()[k].numel() == v.numel()}
        model.load_state_dict(state_dict, strict=False)
        print('Transferred %g/%g items from %s' % (len(state_dict), len(model.state_dict()), weights))  # report
    else:
        # Modified section for our own implementation
        print("device to run on: {}".format(device))
        model = Darknet(opt.cfg,img_size=(opt.img_size,opt.img_size)).to(device) # create
    ...
    if cuda and rank == -1 and torch.cuda.device_count() > 1:
        # Modified section for our own implementation
        print("bypassing dataparallel to be able to do multiple trainings simultainously")
        ...
        if rank in [-1, 0]:
            ...
            for x, tag in zip(list(mloss[:-1]) + list(results) + lr, tags):
                ...
                if wandb:
                    wandb.log({tag: x})  # W&B
                # Modified section for our own implementation
                mlflow.log_metric(tag.replace(':','-'), float(x))
            ...
            if save:
                ...
                if best_fitness == fi:
                    ...
                    mlflow.log_artifact(wdir / 'best_overall.pt', 'weights')
                if best_fitness_p == fi_p:
                    torch.save(ckpt, wdir / 'best_p.pt')
                    mlflow.log_artifact(wdir / 'best_p.pt', 'weights')
                if best_fitness_r == fi_r:
                    torch.save(ckpt, wdir / 'best_r.pt')
                    mlflow.log_artifact(wdir / 'best_r.pt', 'weights')
                if best_fitness_ap50 == fi_ap50:
                    torch.save(ckpt, wdir / 'best_ap50.pt')
                    mlflow.log_artifact(wdir / 'best_ap50.pt', 'weights')
                if best_fitness_ap == fi_ap:
                    torch.save(ckpt, wdir / 'best_ap.pt')
                    mlflow.log_artifact(wdir / 'best_ap.pt', 'weights')
                if best_fitness_f == fi_f:
                    torch.save(ckpt, wdir / 'best_f.pt')
                    mlflow.log_artifact(wdir / 'best_f.pt', 'weights')
                # if epoch == 0:
                #     torch.save(ckpt, wdir / 'epoch_{:03d}.pt'.format(epoch))
                #     mlflow.log_artifact(wdir / 'epoch_{:03d}.pt'.format(epoch), 'weights')
                # if ((epoch+1) % 100) == 0:
                    # torch.save(ckpt, wdir / 'epoch_{:03d}.pt'.format(epoch))
                    # mlflow.log_artifact(wdir / 'epoch_{:03d}.pt'.format(epoch), 'weights')
                # if epoch >= (epochs-5):
                #     torch.save(ckpt, wdir / 'last_{:03d}.pt'.format(epoch))
                #     mlflow.log_artifact(wdir / 'last_{:03d}.pt'.format(epoch), 'weights')
                # elif epoch >= 420: 
                #     torch.save(ckpt, wdir / 'last_{:03d}.pt'.format(epoch))
                #     mlflow.log_artifact(wdir / 'last_{:03d}.pt'.format(epoch), 'weights')
                del ckpt
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mlflow_path', action='store', default='/mnt/mlflow_tracking/mlruns', help='set path to the mlruns directory')
    parser.add_argument('--experiment_name', action='store', default='yolov7', help='mlflow experiment name')
    parser.add_argument('--run_name', action="store", default='', help='Name of the run in mlflow')
    ...
    if not opt.evolve:
        ...
        # Modified section for our own implementation
        if opt.mlflow:
            mlflow.set_tracking_uri('file:{}'.format(opt.mlflow_path))
        ...
        if opt.mlflow:
            ...
            mlflow.log_artifact(opt.cfg, 'attributes')
            mlflow.log_artifact(opt.data, 'attributes')
            mlflow.log_artifact(opt.hyp, 'attributes')
        
        train(hyp, opt, device, tb_writer, wandb)

        # Modified section for our own implementation
        if opt.mlflow:        
            result_path = Path(opt.save_dir)
            for root, dirs, filenames in os.walk(result_path):
                for filename in filenames:
                    if filename.lower().endswith('.jpg') or filename.lower().endswith('.png'):
                        mlflow.log_artifact(os.path.join(root,filename), 'images')
                    elif filename.lower().endswith('.txt') or filename.lower().endswith('.yaml') :
                        mlflow.log_artifact(os.path.join(root,filename), 'attributes')
