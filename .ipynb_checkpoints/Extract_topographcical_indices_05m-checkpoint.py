...
class Topographical_indices:
    ...
    def hillshade(self, input_path, normalized_hillshade, extent):
        ...
        normed_shade = img/32767 #  only values between 0 and 32767 are used for hillshades
    ...
    def maximal_curvature(self, input_path, normalized_max_curvature, extent):
        ...
        img[img > 2] = 2 
        img[img < -2] = -2
        normed_max_curvature = (img--2)/(2--2) 
        utils.WriteGeotiff.write_gtiff(normed_max_curvature, extent, normalized_max_curvature, gdal.GDT_Float32)
    ...
    def multiscale_stddev_normals(self, input_path, normalized_multi_stdon, extent):
        ...
        img = tifffile.imread(self.temp_dir + os.path.basename(input_path))
        img[img > 50] = 50 # check this later
        img[img < 0] = 0
        normed_multi_stdon = img/50
        utils.WriteGeotiff.write_gtiff(normed_multi_stdon, extent, normalized_multi_stdon, gdal.GDT_Float32)  
...
def main(temp_dir, input_path, output_path_hillshade, output_maxelevationdeviation, output_multiscaleelevationpercentile, output_path_minimal_curvature, output_maximal_curvature, output_path_profile_curvature, output_path_stdon, output_path_multiscale_stdon, output_path_elevation_above_pit, output_depthinsink):
#    setup paths
    ...
    for img_path in imgs:
        ...
        max_curvature = os.path.join(output_maximal_curvature,'{}.{}'.format(img_name, 'tif')) 
        ...
        multiscale_std_dev_normals = os.path.join(output_path_multiscale_stdon,'{}.{}'.format(img_name, 'tif'))
        ...
        clean_temp(temp_dir)
        topographical.maximal_curvature(img_path, max_curvature, extent)
        clean_temp(temp_dir)
        ...
        clean_temp(temp_dir)
        topographical.multiscale_stddev_normals(img_path, multiscale_std_dev_normals, extent)
        clean_temp(temp_dir)
...
if __name__ == '__main__':
    ...
    parser.add_argument('output_maximal_curvature', help='directory of where to store maximal curvature images')
    ...
    parser.add_argument('output_path_multiscale_stdon', help='directory to store multiscale stdon images')
